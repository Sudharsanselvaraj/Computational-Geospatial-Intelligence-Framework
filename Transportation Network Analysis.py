...
# 🔵 Fetch ALL heavy rail lines (no over-filtering)
...
# ============================================================
# MTR ROUTES – Smart Label Placement (No Skipping)
# ============================================================
    ...
    if not mtr_visible.empty:
        ...
        if "name" in mtr_visible.columns:
            ...
            for name in unique_names:
                ...
                # ---- Smart Offset if too close ----
                offset_x = 0
                offset_y = 0

                for pt in placed_positions:
                    if midpoint.distance(pt) < 500:
                        offset_y += 150  # move upward slightly

                new_point = Point(midpoint.x + offset_x,
                                  midpoint.y + offset_y)
...
# TITLE
# ============================================================
ax.set_title("SITE ANALYSIS – Transportation",
             fontsize=18,
             weight="bold")

ax.set_axis_off()
plt.tight_layout()
plt.savefig("site_analysis_final_professional.png", dpi=300)
plt.show()
